Shader "Hidden/PostProcessing/Uber (AR)"
{
    HLSLINCLUDE
        ...
        TEXTURE2D_SAMPLER2D(_BackgroundTexY, sampler_BackgroundTexY);
        TEXTURE2D_SAMPLER2D(_BackgroundTexCbCr, sampler_BackgroundTexCbCr);
        ...
        half4 GetBackgroundColor(float2 uv)
        {
            float y = SAMPLE_TEXTURE2D(_BackgroundTexY, sampler_BackgroundTexY, uv).r;
            float4 ycbcr = float4(y, SAMPLE_TEXTURE2D(_BackgroundTexCbCr, sampler_BackgroundTexCbCr, uv).rg, 1.0);
        ...
        half4 FragUber(VaryingsDefault i) : SV_Target
        {
            ...
            float2 uvBackgroundDistored = Distort(i.texcoordBackground);
            //<<<
            ...
            half4 backgroundImage = GetBackgroundColor(uvBackgroundDistored);
            color = lerp(backgroundImage, color, color.a * _ContentOpacity);
            color.a = 1;
            // Gamma space... Gah.
            ...
            #if BLOOM || BLOOM_LOW
            {
                ...
                bloom *= _Bloom_Settings.y * _ContentOpacity;
